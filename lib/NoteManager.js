...
const Directory = require('./dto/Directory');
...
class NoteManager {
  ...
  async write(relativePath, content) {
    ...
    return new Note(relativePath, fileName, 0, false, content);
  }
  ...
  async append(relativePath, content) {
    ...
    const fileName = path.basename(relativePath) + '.md';
    return new Note(relativePath, fileName, 0, false, finalContent);
  }
  ...
  async delete(relativePath) {
    ...
    try {
      ...
      if (stat.isDirectory()) {
        ...
        return new Directory(relativePath, fileName);
      } else {
  ...
  async listDirectory(dir, baseDir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        items.push(new Directory(relativePath, entry.name));
  ...
  async listRecursive(dir, baseDir) {
    ...
    async function scanDir(currentDir, relativePath = '', depth = 0) {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (entry.isDirectory()) {
          items.push(new Directory(itemRelativePath, entry.name, depth, isLast));
          await scanDir(fullPath, itemRelativePath, depth + 1);
        } else if (entry.name.endsWith('.md')) {
          items.push(new Note(itemRelativePath.replace(/\.md$/, ''), entry.name, depth, isLast));
        }
      }
    }
    ...
    try {
      ...
      if (listOnly) {
        ...
        parsed.forEach(item => {
          ...
          if (!structure[folderPath][item.filePath]) {
            structure[folderPath][item.filePath] = [];
          }
          structure[folderPath][item.filePath].push({ lineNum: item.lineNum, text: item.text });
        });

        // Build items array with same structure as listRecursive
        const items = [];
        ...
        sortedFolders.forEach(folder => {
          ...
          if (folder) {
            items.push(new Directory(folder, folder, 0, false));
          }

          const files = Object.keys(structure[folder]).sort();
          files.forEach((filePath, fileIdx) => {
            const fileName = filePath.split('/').pop();
            const isLastFile = fileIdx === files.length - 1;

            // Add file header
            items.push(new Note(filePath, fileName + '.md', folder ? 1 : 0, isLastFile));

            // Add line matches
            const lineMatches = structure[folder][filePath];
            lineMatches.forEach((match, matchIdx) => {
              const isLastMatch = matchIdx === lineMatches.length - 1;
              items.push(new SearchMatch(filePath, match.text, match.lineNum, folder ? 2 : 1, isLastMatch));
            });
          });
        });
  ...
  async quickJournal(content = null) {
    ...
    if (content === null) {
      try {
        ...
        return new Note(journalPath, fileName, 0, false, null);
      }
    }
    ...
    const fileName = dateStr + '.md';
    return new Note(journalPath, fileName, 0, false, finalContent);
  }
}
